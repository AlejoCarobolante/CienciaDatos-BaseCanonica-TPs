...
def tp1_5K10_06():
    ...
    def validate(desde_fuente: str | None, desde_snapshot: str | None) -> str:
        ...
        if len(df) < 300: 
            ...
            # Para una longitud de página de 60 filas o jugadores, tener 348 jugadores implica tener 5 páginas completas y 48 jugadores de la sexta página, por lo cual una longitud mínima de 300 implica que se bajaron 5 páginas completas, y quedan 48 jugadores que pueden haber sido traspasados o dados de baja.

            # El límite de 500 jugadores para el pipeline sin filtro por liga puede deberse a una validación de seguridad, ya que si el HTML cambiase, o si hubiera algún problema de red, se parsearía un número bajo de jugadores respecto al total que debería haber, finalmente mostrando que esa corrida falló y es necesario llevar a cabo alguna acción como reintentar en caso de caída de red o de modificar el dag en caso de que el HTML cambie.
    ...
    @task
    def bundle_zip(csv_path: str, **context) -> str:
        """Genera el ZIP final de entrega con dataset, logs, manifiesto, bronce y DAG."""
        import json
        import zipfile
        import pandas as pd
        ...
        # Identificación del grupo (tomado del ID del DAG o hardcodeado según entrega)
        grupo = "5K10-06"
        integrantes = ["Carobolante, Alejo", "Saini, Alejo", "Podesta, Isidro", "Dallape, Vicenzo"]

        # 1. Medir dimensiones del CSV generado
        df = pd.read_csv(csv_path, low_memory=False)
        filas, columnas = len(df), len(df.columns)
    
        # Extraer metadatos de la liga desde el propio DataFrame o contexto
        league_id = int(params.get("league_id") or df["league_id"].iloc[0])
        league_name = df["league_name"].iloc[0] if "league_name" in df.columns else "Bundesliga"
    
        meta = context["ti"].xcom_pull(task_ids="wait_for_source") or {}
        roster = int(params.get("roster") or meta.get("roster", 260046))

        # 2. Generar contenido de manifiesto.json
        manifiesto_data = {
            "grupo": grupo,
            ...
            "columnas": columnas,
            "run_id": run_id,
            "generado_en": datetime.utcnow().isoformat(timespec="seconds")
        }

        # 3. Listar archivos de la capa bronce utilizados para este roster y liga
        bronze_league_dir = BRONCE_DIR / f"roster={roster}" / f"liga={league_id}"
        bronze_files = []
        if bronze_league_dir.exists():
            bronze_files = sorted([str(p) for p in bronze_league_dir.rglob("*.html.gz")])
        else:
            bronze_files = sorted([str(p) for p in BRONCE_DIR.rglob("*.html.gz")])
        bronce_txt_content = "\n".join(bronze_files)

        # 4. Construir el archivo ZIP
        zip_nombre = f"tp1_{grupo.replace('-', '_').lower()}.zip"
        zip_path = OUTPUT_DIR / zip_nombre
        ...
        with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            ...
            # b. manifiesto.json
            zf.writestr("manifiesto.json", json.dumps(manifiesto_data, indent=2))

            # c. bronce.txt
            zf.writestr("bronce.txt", bronce_txt_content)

            # d. tp1_<comision>_<grupo>.py (archivo fuente del DAG)
            dag_file_path = Path(__file__).resolve()
            zf.write(dag_file_path, arcname=f"tp1_{grupo.replace('-', '_').lower()}.py")

            # e. logs/ de la corrida actual
            logs_base_dir = Path("/usr/local/airflow/logs") / f"dag_id={dag_id}"
            if logs_base_dir.exists():
                for log_file in logs_base_dir.rglob("*.log"):
                    # Si el directorio incluye particiones por run_id, filtrar por la corrida actual
                    if run_id in str(log_file) or f"run_id={run_id}" in str(log_file):
                        rel_path = log_file.relative_to(logs_base_dir)
                        zf.write(log_file, arcname=f"logs/{rel_path}")
                    elif not any("run_id=" in part for part in log_file.parts):
                        # Fallback para estructuras de log sin particionar
                        rel_path = log_file.relative_to(logs_base_dir)
                        zf.write(log_file, arcname=f"logs/{rel_path}")

        log.info("Archivo de entrega generado exitosamente en %s", zip_path)
        return str(zip_path)
